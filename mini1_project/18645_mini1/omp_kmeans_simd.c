...
__inline static
__m128 euclid_dist_2(int    numdims,  /* no. dimensions */
                    float *coord1,   /* [numdims] */
                    float *coord2)   /* [numdims] */
{
    int i;
    __m128 ans= _mm_set1_ps(0.0);
    __m128 first = _mm_set1_ps(0.0);
    for (i=0; i<numdims; i = i + 4){
        first = _mm_sub_ps(_mm_set1_ps(coord1[i]), _mm_set1_ps(coord2[i]));
        __m128 second = _mm_mul_ps(first, first);
        ans = _mm_add_ps(ans, second); 
                         ...
                         float **clusters)    /* [numClusters][numCoords] */
{
    ...
    for (i=1; i<numClusters; i++) {
        ...
        /* no need square root */
        if (_mm_comilt_ss(dist, min_dist)) { /* find the min and its array index */
            min_dist = dist;
            index    = i;
        }
    }
