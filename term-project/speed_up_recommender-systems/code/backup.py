...
output_file = os.path.join('./', "results1.csv")
...
def main():
    ...
    avg = sum(training_rating_list) * 1.0/len(training_rating_list)
    ...
    ratings_matrix = np.ones((number_users + 1, number_movies + 1)) * avg
    valid_ratings_matrix = np.ones((valid_number_users + 1, valid_number_movies + 1)) * avg
